    ...
    useEffect( () => {
      if(interviewId){
        ...
        const fetchFeedbacks = async() => {
          setIsLoading(true);
          try {
            const querSnapRef = query(
              collection(db, "userAnswers"),
              where("userId","==",userId),
              where("mockIdRef","==",interviewId)
            );
  
            const querySnap = await getDocs(querSnapRef);
    ...
    <div className="flex flex-col w-full gap-8 py-5" >
      <div className="flex items-center justify-between w-full gap-2">
        <CustomBreadCrumb 
          ...
          breadCrumpItems={[
            {label: "Mock Interviews", link: "/genrate"},
            {
              label: `${interview?.position}`,
              link: `/genrate/interview/${interview?.id}`,
            },
          ]}
          />
      </div>
                ...
                >
                  ...
                  <AccordionContent className="px-5 py-6 bg-white rounded-b-lg  space-y-5 shadow-inner">
                    ...
                    <Card className="border-none space-y-3 p-4 bg-green-50 rounded-lg shadow-md">
                      ...
                      <CardDescription className="font-medium tex-gray-700 ">
                        {feed.correct_ans}
                      </CardDescription>
                    ...
                    <Card className="border-none space-y-3 p-4 bg-violet-50 rounded-lg shadow-md">
                      <CardTitle className="flex items-center text-lg ">
                        <CircleCheck className="mr-2 text-violet-600" />
                        Feedback
                      </CardTitle>
